...
//			target.x = Mathf.Lerp (transform.position.x, player.position.x, speed * Time.deltaTime);
//		if (Mathf.Abs (transform.position.y - player.position.y) > threshold)
//			target.y = Mathf.Lerp (transform.position.y, player.position.y, speed * Time.deltaTime);
